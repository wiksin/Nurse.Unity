...
namespace Nurse.Unity
{
    ...
    public  static class StringEx
    {
        ...
        /// string 拓展
        /// </summary>
        /// <param name="contextStr">前置字符串</param>
        /// <param name="condition">判断依据</param>
        /// <param name="appendSql">需要添加的字符串</param>
        /// <returns></returns>
        public static string AppendIf(this string contextStr, bool condition, string appendSql)
        {
            return condition ? contextStr + appendSql : contextStr;

        }
        /// <summary>
        ///生成1-指定最大值的随机数
        /// </summary>
        /// <param name="str">前置字符串</param>
        /// <param name="max">最大值默认最大为99</param>
        /// <returns></returns>
        //随机生成两位数值
        public static string ToRandTwo(this string str, int max = 99)
        {

            byte[] buffer = Guid.NewGuid().ToByteArray();//生成字节数组
            int iRoot = BitConverter.ToInt32(buffer, 0);//利用BitConvert方法把字节数组转换为整数
            Random rdmNum = new Random(iRoot);//以这个生成的整数为种子
            return str + rdmNum.Next(1, max).ToString();//生成大于大于a小于b的随机数（int）型
        }
        /// <summary>
        /// 随机长整形
        /// </summary>
